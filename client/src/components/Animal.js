...
export default function Animal({ animal, mySupportedAnimals,  setMySupportedAnimals}) {
    ...
    function handleSupport(e) {
      ...
      function handleDelete(/* characterObj */event) {
        ...
        mySupportedAnimals.forEach(async function (supportedAnimal) {
          console.log(supportedAnimal.details.name);
            if (supportedAnimal.details.name === event.target.parentElement.className) {
                const data = await fetch(
                    'http://localhost:5000/support/delete/' + supportedAnimal._id,
                    { method: 'DELETE' }
                ).then((res) => res.json());
                setMySupportedAnimals((mySupportedAnimals) =>
                    mySupportedAnimals.filter((mySupportedAnimal) => mySupportedAnimal._id !== data.result._id)
                );
            }
        });
      }
